...
## 2026-09-10 - Added index to foreign keys on runs and intents
**Learning:** Missed some foreign keys when adding the index to `phases.run_id` before (`envelopes.run_id`, `artifacts.run_id`, `deliveries.envelope_id`, `bullets.intent_id`). Adding an index on `run_id`, `intent_id`, and `envelope_id` where applicable to avoid linear scan scaling.
**Action:** Always check the whole codebase for any newly discovered un-indexed foreign key.
