...
// Additional indexes on foreign keys to prevent full table scans and N+1 query performance issues
// during lookups and cascading deletes.
...
const runsIntentIDIndex = `CREATE INDEX IF NOT EXISTS idx_runs_intent_id ON runs(intent_id)`
...
func (s *Store) migrateAddIndexes() error {
	...
	if _, err := s.db.Exec(runsIntentIDIndex); err != nil {
		return fmt.Errorf("creating the index on runs.intent_id: %w", err)
	}
