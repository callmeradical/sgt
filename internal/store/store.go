...
// runsProjectIndex makes fetching runs for a project faster.
const runsProjectIndex = `CREATE INDEX IF NOT EXISTS idx_runs_project ON runs(project)`

// intentsProjectIndex makes fetching intents for a project faster.
const intentsProjectIndex = `CREATE INDEX IF NOT EXISTS idx_intents_project ON intents(project)`

// runsStatusUpdatedAtIndex makes fetching runs eligible for cleanup faster.
...
func (s *Store) migrateAddIndexes() error {
	...
	if _, err := s.db.Exec(runsStatusUpdatedAtIndex); err != nil {
		return fmt.Errorf("creating the index on runs.status_updated_at: %w", err)
	}
	return nil
}
